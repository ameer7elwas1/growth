class AuthSystem {
    ...
    // دالة للحصول على إعدادات Supabase مع إعادة المحاولة
    function getSupabaseSettings() {
      // الأولوية 1: window.SUPABASE_CONFIG_LOADED (من config-loader.js)
      if (window.SUPABASE_CONFIG_LOADED && 
          window.SUPABASE_CONFIG_LOADED.URL && 
          window.SUPABASE_CONFIG_LOADED.ANON_KEY) {
        return {
          URL: window.SUPABASE_CONFIG_LOADED.URL,
          ANON_KEY: window.SUPABASE_CONFIG_LOADED.ANON_KEY
        };
      }
      
      // الأولوية 2: window.SUPABASE_CONFIG (من config.js)
      if (window.SUPABASE_CONFIG && 
          window.SUPABASE_CONFIG.URL && 
          window.SUPABASE_CONFIG.ANON_KEY) {
        return {
          URL: window.SUPABASE_CONFIG.URL,
          ANON_KEY: window.SUPABASE_CONFIG.ANON_KEY
        };
      }
      
      // الأولوية 3: SUPABASE_CONFIG من النطاق العام
      ...
      // الأولوية 4: window variables مباشرة
    ...
    const SUPABASE_URL = settings.URL;
    const SUPABASE_ANON_KEY = settings.ANON_KEY;
    
    // التحقق من وجود المفاتيح (فقط إذا كانت فارغة أو placeholders)
    if (!SUPABASE_URL || !SUPABASE_ANON_KEY || 
        SUPABASE_URL === '' || SUPABASE_ANON_KEY === '' ||
        SUPABASE_URL === 'YOUR_SUPABASE_URL_HERE' || 
        SUPABASE_ANON_KEY === 'YOUR_SUPABASE_ANON_KEY_HERE') {
      // فقط أظهر الخطأ مرة واحدة
      if (!window._APP_CONFIG_ERROR_SHOWN) {
        ...
        window._APP_CONFIG_ERROR_SHOWN = true;
      }
    }
